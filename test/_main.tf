...
# call bastion module
module "bastion" {
  source           = "git::https://github.com/OlegGorj/tf-modules-aws-bastion.git?ref=master"
  env              = "${var.env}"
  region           = "${var.region}"
  ...
  subnet_ids       = "${join(",", module.public_subnets.subnet_ids)}"  # "${module.public_subnets.subnet_ids}"
